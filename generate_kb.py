...
BASE_DIR = r"c:\Users\Admin\Desktop\AI_World\Knowledge-Base"

CHAPTERS = [
    ("01-Company.md", "1. Company Specifications", "Founders, Parent Company, Headquarters, Funding, Valuation, Launch Year."),
...
def generate():
    ...
    # 1. Master Index
    index_dir = os.path.join(BASE_DIR, "00-Index")
    os.makedirs(index_dir, exist_ok=True)
    with open(os.path.join(index_dir, "README.md"), "w", encoding="utf-8") as f:
        f.write("""---
...
15. `15-Alternatives.md` — Free, cheap, local offline options.
...
""")
    ...
    # 2. Tool Chapters
    total_files = 0
    for vol_dir, tools in VOLUMES.items():
        vol_path = os.path.join(BASE_DIR, vol_dir)
        os.makedirs(vol_path, exist_ok=True)

        for tool in tools:
            tool_path = os.path.join(vol_path, tool)
            os.makedirs(tool_path, exist_ok=True)

            for filename, title, desc in CHAPTERS:
...
"""
    ...
    print(f"Successfully created {total_files} atomic markdown files across {len(VOLUMES)} Volumes!")
